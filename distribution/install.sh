...
print_usage() {
    ...
    printf "\n"
    printf "  ${YELLOW}1.${NC} Add your API key:\n"
    printf "     ${CYAN}echo 'ANTHROPIC_API_KEY=sk-ant-...' >> ~/.config/context-gateway/.env${NC}\n"
    printf "\n"
    printf "  ${YELLOW}2.${NC} Run the gateway (configs are built-in):\n"
    printf "     ${CYAN}context-gateway${NC}\n"
    printf "\n"
    printf "  ${YELLOW}3.${NC} Or directly launch an agent:\n"
    printf "     ${CYAN}context-gateway agent claude_code${NC}\n"
    printf "\n"
    printf "  ${YELLOW}4.${NC} Optional: export configs for customization:\n"
